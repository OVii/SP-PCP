...
function drawPCPLegend(directory, fileName) {
	...
	// Picture Name
	svg.append("text").attr("x", -20).attr("y", -50).attr("fill", "red").attr(
			"font-size", "25px").attr("font-family", "sans-serif").text(name);
	...
	d3.csv(directory + fileName, function(data) {
		...
		foreground = svg.append("svg:g").attr("class", "foreground").selectAll(
				"path").data(data).enter().append("svg:path").attr("d", path)
				.attr("class", function(d, i) {
					return "pcpRow" + ((i+6)%8+1); // Swap ((i+6)%8+1) vs Not Swap (i+1)
				});
		...
		var legend = svg.selectAll("g.legend").data(data).enter().append("svg:g")
				.attr("class", "legend").attr("transform", function(d, i) {
					return "translate(" + (-10 + i*130) + "," + (350) + ")";
				});

		legend.append("svg:line").attr("class", function(d, i) { return "pcpRow"+(i+1);
		}).attr("x2", 12).attr("stroke-width", 4);
		//.attr("border-color", "#787575")
